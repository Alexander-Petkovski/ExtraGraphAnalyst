...
This is v1.0.0. The core functionality works: fetch data, view the chart, toggle indicators, run predictors, use the console and scripts. A few honest caveats:

There is no settings persistence. Every time you open the app you start fresh with no indicators selected and no remembered ticker. There is no chart export. The predictor output is functional but the confidence scores are rough heuristics rather than rigorous backtests. The Python console has no persistent state between sessions.
